...
<style>
    ...
    .type-toggle label[for="type_{{ CategoryType::INCOME->value}}"] {
  ...
  <div class="row">
    ...
    <div class="col-lg-8">
      ...
      <div class="form-section">
        ...
        <div class="mb-4">
          ...
          <div class="amount-input-group input-group">
            ...
            <input type="text" class="form-control amount-input" id="amount" name="amount" value="{{ old('amount', $transaction->amount->getAmount()->toInt()) }}" placeholder="0" required>
          </div>
          @error('amount')
        ...
        <div class="row">
          <div class="col-md-8 mb-3">
            ...
            <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $transaction->title) }}" placeholder="Contoh: Gaji Bulanan, Belanja Bulanan" required>
            @error('title')
              <div class="text-danger small mt-1">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-4 mb-3">
            ...
            <input type="datetime-local" class="form-control" id="transaction_date" name="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d H:i:s')) }}" required>
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control" id="description" name="description" rows="3" placeholder="Tambahkan catatan untuk transaksi ini...">{{ old('description', $transaction->description) }}</textarea>
          @error('description')
            <div class="text-danger small mt-1">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div class="form-section">
        ...
        <!-- Income Categories -->
        <div id="incomeCategories" class="category-options" style="{{ old('type', $transaction->type) == TransactionType::INCOME ? '' : 'display: none;' }}">
          @forelse($incomeCategories as $category)
            <label class="category-option">
              <input type="radio" name="category_id" value="{{ $category->id }}" @checked(old('category_id', $transaction->category_id) == $category->id)>
        ...
        <!-- Expense Categories -->
        <div id="expenseCategories" class="category-options" style="{{ old('type', $transaction->type) == TransactionType::EXPENSE ? '' : 'display: none;' }}">
              ...
              <div class="d-flex align-items-center w-100">
                ...
                <div class="text-end">
                  ...
                  <div class="progress" style="width: 80px; height: 6px;">
                    <div class="progress-bar {{ $category->has_budget_exceeded ? 'bg-danger' : 'bg-success' }}" style="width: {{ min($category->budget_usage_percentage, 100) }}%">
                    </div>
                  </div>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" id="account_id" name="account_id" required>
            ...
            @foreach($accounts as $account)
              <option value="{{ $account->id }}" data-balance="{{ $account->formatted_current_balance }}" @selected(old('account_id', $transaction->account_id) == $account->id)>
      ...
      <div class="form-section">
        ...
        <div class="mb-3">
          <select class="form-select" id="payment_method" name="payment_method" required>
            ...
            <option value="{{ $payment->value }}" @selected(old('payment_method', $transaction->payment_method) == $payment->value)>{{ $payment->name }}</option>
            @endforeach
          </select>
      ...
      <div class="form-section">
        ...
        <div class="mb-3">
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" role="switch" id="is_recurring" name="is_recurring" value="1" @checked(old('is_recurring', $transaction->is_recurring))>
          ...
          <div id="recurringOptions" style="{{ old('is_recurring', $transaction->is_recurring) ? '' : 'display: none;' }} margin-top: 1rem;">
            <div class="mb-3">
              ...
              <select class="form-select" id="recurring_period" name="recurring_period">
                ...
                <option value="daily" @selected(old('recurring_period', $transaction->recurring_period) == 'daily')>Harian</option>
                <option value="weekly" @selected(old('recurring_period', $transaction->recurring_period) == 'weekly')>Mingguan</option>
                <option value="monthly" @selected(old('recurring_period', $transaction->recurring_period) == 'monthly')>Bulanan</option>
                <option value="yearly" @selected(old('recurring_period', $transaction->recurring_period) == 'yearly')>Tahunan</option>
              </select>
            </div>

            <div class="mb-3">
              <label for="recurring_end_date" class="form-label">Tanggal Berakhir (Opsional)</label>
              <input type="date" class="form-control" id="recurring_end_date" name="recurring_end_date" value="{{ old('recurring_end_date', optional($transaction->recurring_end_date)->format('Y-m-d')) }}">
