      ...
      <div class="card-body">
        <form action="{{ route('apps.budgets.index') }}" method="GET" class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <input type="number" name="year" id="year" class="form-control" min="2020" max="{{ date('Y') + 5 }}" value="{{ $year }}">
          </div>
          <div class="col-md-4">
            ...
            <select name="category_id" id="category_id" class="form-select">
              ...
              @foreach($categories as $category)
                <option value="{{ $category->id }}" 
                  {{ $categoryId == $category->id ? 'selected' : '' }} @selected($categoryId == $category->id)>
                  {{ $category->name }}
                </option>
              @endforeach
            </select>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="progress" style="height: 20px;">
                        <div class="progress-bar bg-{{ $budget->status_color }}" role="progressbar" style="width: {{ min($budget->percentage, 100) }}%;" aria-valuenow="{{ $budget->percentage }}" aria-valuemin="0" aria-valuemax="100">
                    ...
                    <td>
                      <div class="btn-group" role="group">
                        <a href="{{ route('apps.budgets.edit', $budget->id) }}" class="btn btn-sm btn-outline-primary">
                          <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('apps.budgets.destroy', $budget->id) }}" method="POST" class="d-inline">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus anggaran ini?')">
