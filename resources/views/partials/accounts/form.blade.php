<form action="{{ $action }}" method="POST" id="accountForm">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                   ...
                   required 
                   placeholder="Contoh: BCA Tabungan, OVO, Dompet Tunai">
            @error('name')
            ...
            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                ...
                @foreach(\Modules\Wallet\Enums\AccountType::cases() as $type)
                    <option value="{{ $type->value }}" 
                        {{ old('type', isset($account) ? $account->type->value : '') == $type->value ? 'selected' : '' }}>
                        {{ $type->label() }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="row" id="bankInfoSection" style="display: none;">
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="text" 
                   ...
                   placeholder="Contoh: Bank Central Asia, OVO, GoPay">
            @error('bank_name')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <div class="input-group">
                <input type="color" 
                       ...
                       id="color" 
                       name="color" 
                       value="{{ old('color', $account->color ?? '#3490dc') }}"
                       title="Pilih warna untuk akun">
        ...
        <div class="col-md-6 mb-3">
            <label for="icon" class="form-label">Ikon Akun</label>
            <div class="row">
                <div class="col-9">
                ...
                <div class="col-3">
                    <div class="account-icon-preview ms-2" 
                         style="background-color: {{ old('color', $account->color ?? '#3490dc') }}20; 
                                color: {{ old('color', $account->color ?? '#3490dc') }}">
                        <i id="iconPreview" class="{{ old('icon', $account->icon ?? 'bi-wallet') }}"></i>
                    </div>
                </div>
            </div>
            @error('icon')
    ...
    <div class="mb-4">
        ...
        <textarea class="form-control @error('notes') is-invalid @enderror" 
                  ...
                  name="notes" 
                  rows="3"
                  placeholder="Tambahkan catatan atau keterangan tentang akun ini...">{{ old('notes', $account->notes ?? '') }}</textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Listen for changes
        typeSelect.addEventListener('change', toggleBankInfo);
